...
export default function PurchaseManagement({ products, partners, invoices, onDataChange }) {
  ...
  const localParseOcrText = (text) => {
    ...
    for (const line of lines) {
      ...
      if (tokens.length >= 3) {
        let hasAmount = false, hasPrice = false;
        let amountVal = 0, priceVal = 0, qtyVal = 0, tokenIdx = tokens.length - 1;
        ...
        if (hasAmount && tokenIdx >= 0) {
          const pToken = tokens[tokenIdx];
          const cleanP = pToken.replace(/,/g, '');
          const pVal = cleanNumber(pToken);
          if (pVal > 0 && /^\d+$/.test(cleanP)) {
            priceVal = pVal;
            hasPrice = true;
            tokenIdx--;
          }
        ...
        if (hasPrice && tokenIdx >= 0) {
          const qToken = tokens[tokenIdx];
          const cleanQ = qToken.replace(/,/g, '');
          const qVal = cleanFloat(qToken);
          if (qVal > 0 && /^\d+(\.\d+)?$/.test(cleanQ)) {
            qtyVal = qVal;
            tokenIdx--;
          }
        }
