...
router.post('/settings', async (req, res) => {
  const companyId = req.user.company_id;
  const { paperSize, marginTop, marginLeft, fontSize, logoText, hkF2, hkF4, hkF7, hkF8, hkF9, activeHqId } = req.body;
  ...
  try {
    ...
    await db.execute(
      "UPDATE settings SET paper_size = ?, margin_top = ?, margin_left = ?, font_size = ?, logo_text = ?, hk_f2 = ?, hk_f4 = ?, hk_f7 = ?, hk_f8 = ?, hk_f9 = ?, active_hq_id = ? WHERE company_id = ?",
      [
        paperSize || 'A4',
...
router.post('/backup/import', async (req, res) => {
  ...
  try {
    ...
    if (backup.settings) {
      ...
      await db.execute(
        "INSERT INTO settings (company_id, paper_size, margin_top, margin_left, font_size, logo_text, hk_f2, hk_f4, hk_f7, hk_f8, hk_f9, active_hq_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [companyId, s.paper_size || 'A4', s.margin_top || 15, s.margin_left || 15, s.font_size || 10, s.logo_text || '[공급자 보관용]', s.hk_f2 || 'sales', s.hk_f4 || 'save', s.hk_f7 || 'purchase', s.hk_f8 || 'receivables', s.hk_f9 || 'excel-import', s.active_hq_id]
      );
    }
